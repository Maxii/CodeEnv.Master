...
namespace CodeEnv.Master.GameContent {
    ...
    /// 
    /// </summary>
    public class NavTask : AAutoPilotTask {
        ...
        public override bool IsEngaged { get { return _navJob != null; } }

        private bool _doesApProgressCheckPeriodNeedRefresh;
        internal bool DoesApProgressCheckPeriodNeedRefresh {
            private get { return _doesApProgressCheckPeriodNeedRefresh; }
        ...
        internal bool IsIncreaseAboveApSpeedAllowed { private get; set; }

        private EngineRoom _engineRoom;
        private Job _navJob;

        public NavTask(INavTaskClient taskClient, EngineRoom engineRoom) : base(taskClient) {
            _engineRoom = engineRoom;
        }

        public override void RunTask(AutoPilotDestinationProxy destProxy, Action hasArrived = null) {
            ...
            bool isArrived = !destProxy.TryGetArrivalDistanceAndDirection(_taskClient.Position, out directionToArrival, out distanceToArrival);
            D.Assert(!isArrived);
            ...
            if (correctedSpeed != default(Speed)) {
                ...
                _taskClient.ChangeSpeed_Internal(correctedSpeed, _taskClient.IsApCurrentSpeedFleetwide);
            }
            ...
            _navJob = _jobMgr.RecurringWaitForHours(new Reference<GameTimeDuration>(() => progressCheckPeriod), jobName, waitMilestone: () => {
                ...
                if (isArrived = !destProxy.TryGetArrivalDistanceAndDirection(_taskClient.Position, out directionToArrival, out distanceToArrival)) {
                    KillJob();
                    if (hasArrived != null) {
                        hasArrived();
                    }
                ...
                if (CheckForCourseCorrection(directionToArrival, ref previousFrameCourseWasCorrected, ref minFrameWaitBetweenAttemptedCourseCorrectionChecks)) {
                    ...
                    _taskClient.ChangeHeading_Internal(directionToArrival);
                    _taskClient.HandleCourseChanged();  // 5.7.16 added to keep plots current with moving targets
                }
        ...
        private GameTimeDuration GenerateProgressCheckPeriod(float distanceToArrival, out Speed correctedSpeed) {
            ...
            if (hoursPerCheckPeriod < MinHoursPerProgressCheckPeriodAllowed) {
                ...
                speed = _taskClient.CurrentSpeedSetting;
                while (hoursPerCheckPeriod < MinHoursPerProgressCheckPeriodAllowed) {
                    Speed slowerSpeed;
                    if (speed.TryDecreaseSpeed(out slowerSpeed)) {
                        float slowerSpeedValue = _taskClient.IsApCurrentSpeedFleetwide ? slowerSpeed.GetUnitsPerHour(_taskClient.UnitFullSpeedValue) : slowerSpeed.GetUnitsPerHour(_taskClient.FullSpeedValue);
        ...
        private bool CheckForCourseCorrection(Vector3 directionToDest, ref int lastFrameCorrected, ref int minFrameWait) {
            ...
            if (currentFrame < lastFrameCorrected + minFrameWait) {
                ...
                // do a check
                float reqdCourseCorrectionDegrees = Vector3.Angle(_taskClient.IntendedHeading, directionToDest);
        ...
        private bool TryCheckForPeriodOrSpeedCorrection(float distanceToArrival, bool isIncreaseAboveApSpeedAllowed, float halfArrivalCaptureDepth,
            ...
            if (DoesApProgressCheckPeriodNeedRefresh) {
                ...
                DoesApProgressCheckPeriodNeedRefresh = false;
                return true;
            }
            ...
            if (checksRemainingBeforeArrival < checksRemainingThreshold) {
                ...
                if (isDistanceCoveredPerCheckTooHigh) {
                    ...
                    if (_taskClient.CurrentSpeedSetting.TryDecreaseSpeed(out correctedSpeed)) {
                ...
                if (checksRemainingBeforeArrival > MinNumberOfProgressChecksBeforeSpeedIncreasesCanBegin) {
                    if (isIncreaseAboveApSpeedAllowed || _taskClient.CurrentSpeedSetting < _taskClient.ApSpeed) {
                        if (_taskClient.CurrentSpeedSetting.TryIncreaseSpeed(out correctedSpeed)) {
