...
namespace CodeEnv.Master.GameContent {
    ...
    /// 
    /// </summary>
    public class SystemData2 : AItemData2 {
        ...
        private SettlementCmdData2 _settlementData;
        public SettlementCmdData2 SettlementData {
            get { return _settlementData; }
            set { SetProperty<SettlementCmdData2>(ref _settlementData, value, "SettlementData", OnSettlementDataChanged); }
        }

        private StarData2 _starData;
        public StarData2 StarData {
            get { return _starData; }
            set { SetProperty<StarData2>(ref _starData, value, "StarData", OnStarDataChanged, OnStarDataChanging); }
        }

        private IList<PlanetoidData2> _allPlanetoidData = new List<PlanetoidData2>();

        private IDictionary<PlanetoidData2, IList<IDisposable>> _planetoidSubscribers;
        ...
        public SystemData2(string systemName, Index3D sectorIndex, Topography topography)
            : base(systemName) {
        ...
        private void Subscribe() {
            _planetoidSubscribers = new Dictionary<PlanetoidData2, IList<IDisposable>>();
            _starSubscribers = new List<IDisposable>();
        }

        public void AddPlanetoid(PlanetoidData2 data) {
            _allPlanetoidData.Add(data);
            SubscribeToPlanetoidDataValueChanges(data);
            RecalcAllProperties();
        }

        public bool RemovePlanetoid(PlanetoidData2 data) {
            bool isRemoved = _allPlanetoidData.Remove(data);
            if (!isRemoved) {
                D.Warn("Attempting to remove {0}.{1} that is not present.", data.FullName, typeof(PlanetoidData2));
                return false;
            }
        ...
        private void OnStarDataChanging(StarData2 newData) {
        ...
        private void SubscribeToPlanetoidDataValueChanges(PlanetoidData2 data) {
            ...
            planetSubscriber.Add(data.SubscribeToPropertyChanged<PlanetoidData2, int>(pd => pd.Capacity, OnSystemMemberCapacityChanged));
            planetSubscriber.Add(data.SubscribeToPropertyChanged<PlanetoidData2, OpeYield>(pd => pd.Resources, OnSystemMemberResourceValueChanged));
            planetSubscriber.Add(data.SubscribeToPropertyChanged<PlanetoidData2, XYield>(pd => pd.SpecialResources, OnSystemMemberSpecialResourceValueChanged));
        }

        private void SubscribeToStarDataValueChanges() {
            _starSubscribers.Add(StarData.SubscribeToPropertyChanged<StarData2, int>(sd => sd.Capacity, OnSystemMemberCapacityChanged));
            _starSubscribers.Add(StarData.SubscribeToPropertyChanged<StarData2, OpeYield>(sd => sd.Resources, OnSystemMemberResourceValueChanged));
            _starSubscribers.Add(StarData.SubscribeToPropertyChanged<StarData2, XYield>(sd => sd.SpecialResources, OnSystemMemberSpecialResourceValueChanged));
        }
        ...
        private void SubscribeToSettlementDataValueChanges() {
            ...
            _settlementSubscribers.Add(SettlementData.SubscribeToPropertyChanged<SettlementCmdData2, Player>(sd => sd.Owner, OnSettlementOwnerChanged));
        }

        private void UnsubscribeToPlanetoidDataValueChanges(PlanetoidData2 data) {
        ...
        private void Unsubscribe() {
            IList<PlanetoidData2> subscriberKeys = new List<PlanetoidData2>(_planetoidSubscribers.Keys);
            // copy of key list as you can't remove keys from a list while you are iterating over the list
            foreach (PlanetoidData2 data in subscriberKeys) {
                UnsubscribeToPlanetoidDataValueChanges(data);
            }
