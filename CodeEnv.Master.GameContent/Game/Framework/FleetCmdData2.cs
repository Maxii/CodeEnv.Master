...
namespace CodeEnv.Master.GameContent {
    ...
    /// 
    /// </summary>
    public class FleetCmdData2 : ACommandData2 {
        ...
        public new ShipData2 HQElementData {
            get { return base.HQElementData as ShipData2; }
        ...
        public FleetCmdData2(FleetCmdStat stat)
        ...
        protected override void UpdateComposition() {
            var elementCategories = ElementsData.Cast<ShipData2>().Select(sd => sd.Category);
        ...
        private void UpdateFullSpeed() {
            if (ElementsData.Any()) {
                UnitFullStlSpeed = ElementsData.Min(eData => (eData as ShipData2).FullStlSpeed);
                UnitFullFtlSpeed = ElementsData.Min(eData => (eData as ShipData2).FullFtlSpeed);
                UnitFullSpeed = ElementsData.Min(eData => (eData as ShipData2).FullSpeed);
            }
        }

        private void UpdateMaxTurnRate() {
            if (ElementsData.Any()) {
                UnitMaxTurnRate = ElementsData.Min(data => (data as ShipData2).MaxTurnRate);
            }
        }

        protected override void Subscribe(AElementData2 elementData) {
            base.Subscribe(elementData);
            IList<IDisposable> anElementsSubscriptions = _subscribers[elementData];
            ShipData2 shipData = elementData as ShipData2;
            anElementsSubscriptions.Add(shipData.SubscribeToPropertyChanged<ShipData2, float>(ed => ed.FullStlSpeed, OnShipFullSpeedChanged));
            anElementsSubscriptions.Add(shipData.SubscribeToPropertyChanged<ShipData2, float>(ed => ed.FullFtlSpeed, OnShipFullSpeedChanged));
            anElementsSubscriptions.Add(shipData.SubscribeToPropertyChanged<ShipData2, bool>(ed => ed.IsFtlAvailableForUse, OnShipFtlAvailableForUseChanged));
            anElementsSubscriptions.Add(shipData.SubscribeToPropertyChanged<ShipData2, float>(ed => ed.MaxTurnRate, OnShipElementMaxTurnRateChanged));
        }
