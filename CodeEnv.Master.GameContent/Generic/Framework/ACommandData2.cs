...
namespace CodeEnv.Master.GameContent {
    ...
    /// 
    /// </summary>
    public abstract class ACommandData2 : AMortalItemData2 {
        ...
        private AElementData2 _hqElementData;
        public AElementData2 HQElementData {
            get { return _hqElementData; }
            set { SetProperty<AElementData2>(ref _hqElementData, value, "HQElementData", OnHQElementDataChanged); }
        }
        ...
        protected IList<AElementData2> ElementsData { get; private set; }
        protected IDictionary<AElementData2, IList<IDisposable>> _subscribers;
        ...
        public ACommandData2(string unitName, float cmdMaxHitPoints)
        ...
        private void InitializeCollections() {
            ElementsData = new List<AElementData2>();
            _subscribers = new Dictionary<AElementData2, IList<IDisposable>>();
        }
        ...
        public virtual void AddElement(AElementData2 elementData) {
            D.Assert(!ElementsData.Contains(elementData), "Attempted to add {0} {1} that is already present.".Inject(typeof(AElementData2).Name, elementData.ParentName));
        ...
        private void VerifyOwner(AElementData2 elementData) {
        ...
        private void UpdateElementParentName(AElementData2 elementData) {
        ...
        public virtual bool RemoveElement(AElementData2 elementData) {
            D.Assert(ElementsData.Contains(elementData), "Attempted to remove {0} {1} that is not present.".Inject(typeof(AElementData2).Name, elementData.ParentName));
        ...
        private void RecalcUnitMaxHitPoints() {
            UnitMaxHitPoints = ElementsData.Sum<AElementData2>(ed => ed.MaxHitPoints);
        }

        private void RecalcUnitCurrentHitPoints() {
            UnitCurrentHitPoints = ElementsData.Sum<AElementData2>(ed => ed.CurrentHitPoints);
        }
        ...
        protected virtual void Subscribe(AElementData2 elementData) {
            _subscribers.Add(elementData, new List<IDisposable>());
            IList<IDisposable> anElementsSubscriptions = _subscribers[elementData];
            anElementsSubscriptions.Add(elementData.SubscribeToPropertyChanged<AElementData2, float>(ed => ed.CurrentHitPoints, OnElementCurrentHitPointsChanged));
            anElementsSubscriptions.Add(elementData.SubscribeToPropertyChanged<AElementData2, float>(ed => ed.MaxHitPoints, OnElementMaxHitPointsChanged));
            anElementsSubscriptions.Add(elementData.SubscribeToPropertyChanged<AElementData2, CombatStrength>(ed => ed.DefensiveStrength, OnElementDefensiveStrengthChanged));
            anElementsSubscriptions.Add(elementData.SubscribeToPropertyChanged<AElementData2, CombatStrength>(ed => ed.OffensiveStrength, OnElementOffensiveStrengthChanged));
            anElementsSubscriptions.Add(elementData.SubscribeToPropertyChanged<AElementData2, float>(ed => ed.MaxWeaponsRange, OnElementMaxWeaponsRangeChanged));
            anElementsSubscriptions.Add(elementData.SubscribeToPropertyChanged<AElementData2, float>(ed => ed.MaxSensorRange, OnElementMaxSensorRangeChanged));
        }
        ...
        private void Unsubscribe(AElementData2 elementData) {
        ...
        protected override void Unsubscribe() {
            base.Unsubscribe();
            IList<AElementData2> subscriberKeys = new List<AElementData2>(_subscribers.Keys);
            // copy of key list as you can't remove keys from a list while you are iterating over the list
            foreach (AElementData2 eData in subscriberKeys) {
                Unsubscribe(eData);
            }
            _subscribers.Clear();
        }
