        ...
        if (!tType.IsValueType) {
            if (value != null) {
                if (tType == typeof(string)) {
                    Debug.LogWarningFormat("{0} BackingStore {1} and value {2} are equal. Property not changed.", propertyName, backingStore, value);
                }
                else {
                    Debug.LogWarningFormat("{0} BackingStore and value of Type {1} are equal. Property not changed.", propertyName, tType.Name);
                }
            }
        }
    }
