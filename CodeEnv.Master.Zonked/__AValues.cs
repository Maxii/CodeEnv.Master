...
namespace CodeEnv.Master.Common {
    ...
    [Obsolete]      // implemented with the old XmlDocument paradigm
    public abstract class __AValues<T> where T : class {
        ...
        private string _booleanTagXPath;
        private string _intTagXPath;
        private string _floatTagXPath;
        ...
        protected virtual void Initialize() {
            _booleanTagXPath = "/" + RootTagName + "/Boolean";
            _intTagXPath = "/" + RootTagName + "/Integer";
            _floatTagXPath = "/" + RootTagName + "/Float";

            _document = LoadAndValidateDocument();
        }
        ...
        private XmlDocument LoadAndValidateDocument() {
            ...
            D.Log("The path to the XmlDocument is {0}.", xmlDocumentPath);
        ...
        protected void InitializePropertyValues() {
            XmlNode topNode = _document.SelectSingleNode(_booleanTagXPath);
            XmlNodeList settingNodes = topNode.SelectNodes(_settingTagName);
            ...
            topNode = _document.SelectSingleNode(_intTagXPath);
            settingNodes = topNode.SelectNodes(_settingTagName);
            foreach (XmlNode settingNode in settingNodes) {
                XmlNode propertyNameNode = settingNode.FirstChild;
                string propertyName = propertyNameNode.InnerText;
            ...
            topNode = _document.SelectSingleNode(_floatTagXPath);
            settingNodes = topNode.SelectNodes(_settingTagName);
            foreach (XmlNode settingNode in settingNodes) {
                XmlNode propertyNameNode = settingNode.FirstChild;
                string propertyName = propertyNameNode.InnerText;
        ...
        private void AssignValueToProperty(string propertyName, float value) {
            ...
            Action<float> propertySet = (Action<float>)Delegate.CreateDelegate(typeof(Action<float>), Instance, propertyInfo.GetSetMethod(true));
            propertySet(value);
        }
        ...
        private void AssignValueToProperty(string propertyName, int value) {
            ...
            Action<int> propertySet = (Action<int>)Delegate.CreateDelegate(typeof(Action<int>), Instance, propertyInfo.GetSetMethod(true));
            propertySet(value);
        }
        ...
        private void AssignValueToProperty(string propertyName, bool value) {
            ...
            Action<bool> propertySet = (Action<bool>)Delegate.CreateDelegate(typeof(Action<bool>), Instance, propertyInfo.GetSetMethod(true));
            propertySet(value);
        }
