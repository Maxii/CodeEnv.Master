...
public class ColliderRelay : AMonoBase {
    ...
    void Awake() {
        if (relayTargets.Length == 0) {
            ...
            Logger.Log("Parent {0} auto assigned as {1}.".Inject(parent.name, typeof(IColliderRelayTarget)));
        }

        foreach (var t in relayTargets) {
            if (t.GetInterface<IColliderRelayTarget>() == null) {
                D.Warn("Assigned GameObject {0} is not of Type {1}.".Inject(t.gameObject.name, typeof(IColliderRelayTarget).Name));
                return;
            }
        }
    }
