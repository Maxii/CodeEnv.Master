...
/// <summary>
/// Creates a system at the location of this script's GameObject. Used for programmatic (auto) deployment
/// of Systems, and as the base class for DebugSystemCreator.
...
/// </summary>
[System.Obsolete]
public class SystemCreatorOld : AMonoBase {
    ...
    protected virtual bool IsCompositionPreset { get { return false; } }

    protected virtual int MaxPlanets { get { return MaxPlanetsPerSystem; } }

    protected virtual int MaxMoons { get { return MaxMoonsPerSystem; } }

    protected virtual int CMsPerPlanetoid { get { return 1; } }

    protected virtual bool IsTrackingLabelEnabled { get { return DebugControls.Instance.AreAutoSystemCreatorTrackingLabelsEnabled; } }
    ...
    private IList<PlanetStat> CreateRandomPlanetStats() {
        D.Assert(!IsCompositionPreset);
        var planetStats = new List<PlanetStat>();
        int planetCount = MaxPlanetsPerSystem;
    ...
    private void MakeStar() {
        ...
        if (IsCompositionPreset) {
            ...
            _star = _systemFactory.MakeInstance(_starStat, cameraStat, _system.gameObject, SystemName);
        }
    }
    ...
    private void AssignSystemOrbitSlotsToPlanets() {
        ...
        OrbitData[] allSystemOrbitSlots = GenerateAllSystemOrbitSlots(settlementOrbitSlotIndex, out _systemOrbitSlotDepth);
        ...
        foreach (var planet in _planets) {
            ...
            if (TryFindOrbitSlot(out slotIndex, slots)) {
                OrbitData planetOrbitData = allSystemOrbitSlots[slotIndex];
                // assign the planet's name using its orbital slot
                planet.Name = SystemName + Constants.Space + _planetNumbers[slotIndex];
                _generalFactory.InstallCelestialItemInOrbit(planet.gameObject, planetOrbitData);
    ...
    private void MakeMoons() {
        ...
        if (IsCompositionPreset) {
            ...
            _moons = new List<MoonItem>(MaxMoonsPerSystem);
    ...
    /// <returns></returns>
    //public static bool TryGetSystem(IntVector3 sectorIndex, out SystemItem system) {
    ...
    //private static IDictionary<IntVector3, SystemItem> _systemLookupBySectorIndex = new Dictionary<IntVector3, SystemItem>();
