...
public abstract class AUnitCreator : AMonoBase, IDateMinderClient {
    ...
    protected virtual void AttemptToAuthorizeDeploymentOnStart() {
        if (_gameMgr.IsRunning) {
            __ValidateNotDuplicateDeployment();
            D.Warn("{0}.Start is authorizing deployment.", DebugName);  // 11.13.17 OPTIMIZE haven't seen this so far
            PrepareUnitForDeployment();
            AuthorizeDeployment();
        }
    }
    ...
    private void DeployOnDeployDate() {
        ...
        GameTime.Instance.DateMinder.Add(DeployDate, this);
    }
    ...
    private void Deploy() { // 3.25.16 wait approach changed from dateChanged event handler to WaitForDate utility method
        ...
        if (currentDate > DeployDate) {
            D.Log(ShowDebugLog, "{0} exceeded DeployDate {1}, so actually deployed on {2}.", DebugName, DeployDate, currentDate);
        }
        else {
    ...
    [System.Diagnostics.Conditional("DEBUG")]
    protected abstract void __ValidateNotDuplicateDeployment();
    ...
    ///// The only exception is the AutoSettlementCreator which positions its unit into orbit around
    ///// its parent system.
    ...
    ///// <summary>
    ///// Clears any element references so that the garbage collector can collect the element
    ///// if/when it dies. No need for clearing a Command reference as this creator is destroyed when the Cmd dies.
    ///// </summary>
    //protected abstract void ClearElementReferences();
