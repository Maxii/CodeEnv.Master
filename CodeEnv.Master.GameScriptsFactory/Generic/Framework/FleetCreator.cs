...
/// </summary>
public class FleetCreator : ACreator<ShipItem, FleetItem, FleetComposition> {
    ...
    //        ShipCategory hull = DeriveCategory(ship);
    ...
    protected override void CreateCompositionFromChildren() {
        ...
        foreach (var element in _elements) {
            ShipCategory category = DeriveCategory(element);
            string elementName = element.gameObject.name;
            ShipData elementData = CreateElementData(category, elementName, owner);
    ...
    protected override void CreateRandomComposition() {
        ...
        ShipCategory[] validCategories = new ShipCategory[] { ShipCategory.Frigate, ShipCategory.Destroyer, ShipCategory.Cruiser, ShipCategory.Carrier, ShipCategory.Dreadnaught };

        int elementCount = RandomExtended<int>.Range(1, maxElements);
        for (int i = 0; i < elementCount; i++) {
            ShipCategory elementCategory = RandomExtended<ShipCategory>.Choice(validCategories);
            ...
            ShipData elementData = CreateElementData(elementCategory, elementInstanceName, owner);
            _composition.Add(elementData);
        }

    }

    private ShipData CreateElementData(ShipCategory elementCategory, string elementInstanceName, IPlayer owner) {
    ...
    protected override void BuildElements() {
        ...
        foreach (var elementCategory in _composition.Categories) {
            GameObject elementPrefabGo = RequiredPrefabs.Instance.ships.First(f => f.gameObject.name == elementCategory.GetName()).gameObject;
            ...
            _composition.GetData(elementCategory).ForAll(data => {
                ...
                ShipItem element = elementGoClone.GetSafeMonoBehaviourComponent<ShipItem>();
    ...
    protected override void InitializeElements() {
        IDictionary<ShipCategory, Stack<ShipData>> dataStackLookup = new Dictionary<ShipCategory, Stack<ShipData>>();
        foreach (var element in _elements) {
            ShipCategory elementCategory = DeriveCategory(element);
            Stack<ShipData> elementDataStack;
            if (!dataStackLookup.TryGetValue(elementCategory, out elementDataStack)) {
                elementDataStack = new Stack<ShipData>(_composition.GetData(elementCategory));
    ...
    protected override void InitializePiece() {
        _command.Data = new FleetData(_pieceName);    // automatically adds the command transform to Data when set
    ...
    private int GetCurrentCount(ShipCategory elementCategory) {
