...
public class GuiManager : AMonoBaseSingleton<GuiManager>, IDisposable {
    ...
    /// <summary>
    /// Restores all panels in preparation for a scene change.
    /// </summary>
    private void RestoreAllPanels() {
        D.Log("Restoring all Invisible Panels in preparation for scene change.");
        for (int i = 0; i < _stackedPanelsToRestore.Count; i++) {
            var panelsToRestore = _stackedPanelsToRestore.Pop();
    ...
    private void OnGuiVisibilityChange(GuiVisibilityButton.GuiVisibilityChangeEvent e) {
        ...
        switch (e.GuiVisibilityCmd) {
            case GuiVisibilityCommand.HideVisibleGuiPanels:
                var activeUIRootChildPanelCandidates = gameObject.GetSafeMonoBehaviourComponentsInChildren<UIPanel>().Except(_panelsToAlwaysRemainActive);
                var panelsToDeactivate = activeUIRootChildPanelCandidates.Except(e.Exceptions);
                //var panelsToDeactivate = (from p in activeUIRootChildPanelCandidates where !e.Exceptions.Contains<UIPanel>(p) select p);
                panelsToDeactivate.ForAll<UIPanel>(p => {
