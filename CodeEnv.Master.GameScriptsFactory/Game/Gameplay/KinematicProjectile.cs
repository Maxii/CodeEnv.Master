...
// </copyright> 
// <summary> 
// File: KinematicProjectile.cs
// Unguided AKinematicProjectileOrdnance containing effects for muzzle flash, inFlight operation and impact.  
// </summary> 
// -------------------------------------------------------------------------------------------------------------------- 
...
/// Unguided AKinematicProjectileOrdnance containing effects for muzzle flash, inFlight operation and impact.  
/// </summary>
public class KinematicProjectile : AKinematicProjectileOrdnance {
    ...
    public override void Launch(IElementAttackable target, AWeapon weapon, Topography topography) {
        ...
        //D.Log(ShowDebugLog, "{0} is launching against {1} with a TgtBearingError = {2:0.#}.", DebugName, Target.DebugName,
    ...
    /// <summary>
    /// One-time initialization of the forward speed of this 'projectile'.
    /// </summary>
    private void InitializeSpeed() {
        _currentSpeed = MaxSpeed * _gameTime.GameSpeedAdjustedHoursPerSecond;
    }

    void FixedUpdate() {
        if (_toConductMovement) {
            _rigidbody.MovePosition(transform.position + CurrentHeading * _currentSpeed * _gameTime.DeltaTime);
        }
    }
    ...
    protected override void HandleCollision(GameObject impactedGo, ContactPoint contactPoint) {
        ...
        if (impactedTarget != null) {
            ...
            D.Warn("{0} distance to intended Target on collision with {1} = {2:0.#}.", DebugName, impactedGo.name, Vector3.Distance(transform.position, Target.Position));
        }
